...
//      cell versions, then serially drives five root funcIndex values through
//      AOT, 64 real Tier-1 dispatches, Tier-2 publication, and Ready calls.
...
#define EJIT_BOUND_MULTI_T1_WAIT_ROUNDS 6000u
#define EJIT_BOUND_MULTI_T2_WAIT_ROUNDS 30000u
...
// The helper owns an independent cell/trp identity and repeats both borrowed
// pointer contracts so its wrapper can carry descriptors on direct calls.
...
ejit_entry uint32_t bound_multi_root0(
    EJIT_DIM(cell) uint8_t cellIndex, EJIT_DIM(trp) uint8_t trpIndex,
    EJIT_BOUND_PTR(cell) const EJitBoundMultiCellConfig *cellConfig,
    EJIT_BOUND_PTR(trp) const EJitBoundMultiTrpConfig *trpConfig,
    uint32_t input) {
...
#define DEFINE_BOUND_MULTI_ROOT(N)                                             \
  ejit_entry uint32_t bound_multi_root##N(                                     \
      EJIT_DIM(cell) uint8_t cellIndex, EJIT_DIM(trp) uint8_t trpIndex,        \
      EJIT_BOUND_PTR(cell) const EJitBoundMultiCellConfig *cellConfig,         \
      EJIT_BOUND_PTR(trp) const EJitBoundMultiTrpConfig *trpConfig,            \
      uint32_t input) {                                                        \
    return bound_multi_helper(cellIndex, trpIndex, cellConfig, trpConfig,      \
                              input);                                          \
  }

DEFINE_BOUND_MULTI_ROOT(1)
DEFINE_BOUND_MULTI_ROOT(2)
DEFINE_BOUND_MULTI_ROOT(3)
DEFINE_BOUND_MULTI_ROOT(4)
...
static const char *target_name(uint32_t target) {
  static const char *const names[5] = {"root0", "root1", "root2", "root3",
                                       "root4"};
  return target < 5u ? names[target] : "invalid";
}

static uint32_t call_bound_target(uint32_t target, uint32_t cell) {
  const uint8_t cell8 = (uint8_t)cell;
  const uint8_t trp8 = (uint8_t)EJIT_BOUND_MULTI_TRP;
  const EJitBoundMultiCellConfig *cellConfig =
      &g_bound_multi_cell_configs[cell];
  const EJitBoundMultiTrpConfig *trpConfig =
      &g_bound_multi_trp_configs[EJIT_BOUND_MULTI_TRP];
  switch (target) {
  case BOUND_MULTI_ROOT0:
    return bound_multi_root0(cell8, trp8, cellConfig, trpConfig, 10u);
  case BOUND_MULTI_ROOT1:
    return bound_multi_root1(cell8, trp8, cellConfig, trpConfig, 10u);
  case BOUND_MULTI_ROOT2:
    return bound_multi_root2(cell8, trp8, cellConfig, trpConfig, 10u);
  case BOUND_MULTI_ROOT3:
    return bound_multi_root3(cell8, trp8, cellConfig, trpConfig, 10u);
  case BOUND_MULTI_ROOT4:
    return bound_multi_root4(cell8, trp8, cellConfig, trpConfig, 10u);
  default:
    return 0u;
  }
}
...
static int wait_for_t1(uint64_t baseline, uint32_t target, uint32_t cell) {
  ...
  for (uint32_t round = 0; round < EJIT_BOUND_MULTI_T1_WAIT_ROUNDS; ++round) {
    ...
    const uint32_t delayRc = SRE_TaskDelay(EJIT_BOUND_MULTI_WAIT_TICKS);
    if (delayRc != 0u) {
      SRE_printf("[BOUND-MULTI] FAIL SRE_TaskDelay(%u) rc=%u during T1 wait\n",
                 EJIT_BOUND_MULTI_WAIT_TICKS, delayRc);
      return 0;
    }
  }
  ejit_taskpool_stats_t stats = {0};
...
static int drive_profile(uint32_t target, uint32_t cell, uint64_t *sink) {
  ...
  *sink ^= got;
  ...
  // The 64th real Tier-1 dispatch has already queued Tier-2. Stop calling the
  // wrapper while it is pending: a tight producer loop can starve a low-priority
  // fixed worker on some SRE schedulers and is not needed to make progress.
  for (uint32_t round = 0; round < EJIT_BOUND_MULTI_T2_WAIT_ROUNDS; ++round) {
    const uint32_t delayRc = SRE_TaskDelay(EJIT_BOUND_MULTI_WAIT_TICKS);
    if (delayRc != 0u) {
      SRE_printf("[BOUND-MULTI] FAIL SRE_TaskDelay(%u) rc=%u during T2 wait\n",
                 EJIT_BOUND_MULTI_WAIT_TICKS, delayRc);
      return 0;
    }
    ...
    if (stats.compileFailed || stats.publishFailed) {
      print_profile_state("T2-failed", target, cell,
                          EJIT_BOUND_MULTI_T1_SAMPLES, baseline,
                          &stats);
      return 0;
    }
    if (stats.asyncCompiles >= baseline + 2u && stats.pendingEntries == 0 &&
      ...
      *sink ^= verify2;
      print_profile_state("T2-ready", target, cell,
                          EJIT_BOUND_MULTI_T1_SAMPLES + 2u, baseline,
                          &stats);
      return 1;
    }
    if (round == 0u || ((round + 1u) % 1000u) == 0u)
...
static int run_producer(void) {
  ...
  const uint32_t targets[6] = {
      BOUND_MULTI_ROOT0, BOUND_MULTI_ROOT0, BOUND_MULTI_ROOT1,
      BOUND_MULTI_ROOT2, BOUND_MULTI_ROOT3, BOUND_MULTI_ROOT4};
  const uint32_t cells[6] = {
      EJIT_BOUND_MULTI_CELL_A, EJIT_BOUND_MULTI_CELL_B,
      EJIT_BOUND_MULTI_CELL_A, EJIT_BOUND_MULTI_CELL_A,
      EJIT_BOUND_MULTI_CELL_A, EJIT_BOUND_MULTI_CELL_A};
  for (uint32_t i = 0; i < 6u; ++i) {
    const uint32_t cell = cells[i];
    // Each identity reaches Ready before the next request. In particular,
    // root0's two cell versions are never in flight concurrently.
    if (!drive_profile(targets[i], cell, &sink)) {
  ...
  if (after.asyncCompiles < before.asyncCompiles + 12u || after.compileFailed ||
      after.publishFailed || ejit_taskpool_pending_count() != 0) {
    SRE_printf("[BOUND-MULTI][core=%u] FAIL final compiles=%llu/12 "
  ...
  __atomic_store_n(&g_bound_multi_stage, BOUND_MULTI_COMPILED,
                   __ATOMIC_RELEASE);
  SRE_printf("[BOUND-MULTI][core=%u] PASS five roots T2 Ready; root0 cells=%u/%u "
...
int test_ejit_bound_ptr_multistruct_print(uint8_t a, uint8_t b, uint8_t c,
  ...
  ejit_print_dumped("bound_multi_root0");
  SRE_printf("[BOUND-MULTI][core=%u] === ROOT4 ENTRY VIEW ===\n", core);
  ejit_print_dumped("bound_multi_root4");
  SRE_printf("[BOUND-MULTI][core=%u] === ROOT MODULE VIEW ===\n", core);
  ejit_print_dumped_module("bound_multi_root0");
  SRE_printf("[BOUND-MULTI][core=%u] PASS sink=0x%llx; expected five root "
             "funcIndex values, root0 cell A/B, and no payload copy\n"
